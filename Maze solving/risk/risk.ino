...
int turnDelay=600,forwardDelay=220;
int buzzer=12;
int nodeCount=0;
...
int leftIR=200,rightIR=200,frontIR;
int leftInnerIR=200,rightInnerIR=200;
...
void RLreturn()
{
    ...
    motor(leftSpeed,rightSpeed);
    sensorMapping();
    if(mappedValue<=6 && leftIR==0 && rightIR==1)
    {
      motor(fSpeed,fSpeed);
      delay(forwardDelay);
      brake();
      ...
      else
      {
        ...
        motor(maxSpeed,maxSpeed);
        sensorMapping();
        Serial.print("Returning to loop");
        loop();
      }
    } 
    else if(mappedValue>=8 && rightIR==0 && leftIR==1)
    {
